...
Run the `yazzi-ui` init command to setup your project:

```bash
npx yazzi-ui@latest init
```
...
When running `npx yazzi-ui@latest init`, your tailwind config for content will be overwritten. To fix this, change the `module.exports` to `export default` and the `content` section with the code below to your `tailwind.config.mjs` file:
...
```bash
npx yazzi-ui@latest add button
```
